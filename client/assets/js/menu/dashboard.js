function dashboard(alert = null) {
	...
	const modalTambah = (data) => {
		...
		data.jenis.forEach(function (el) {
			jenis += `<option value="${el.id}">${el.title}</option>`;
		});
		let bahasa = ``;
		data.bahasa.forEach(function (el) {
			bahasa += `<option value="${el.id}">${el.title}</option>`;
		});
		let tingkat = ``;
		data.tingkat.forEach(function (el) {
			tingkat += `<option value="${el.id}">${el.title}</option>`;
		});
		...
		$.ajax({
			...
			success: result => {
				if (result.response) {
					...
					cekKoneksi = true;
				} else dashboard({ icon: 'error', text: "gagal menghapus data" });
			}
		});
	};
	setTimeout(act, 500);
	...
	const act = () => {
		...
		$.ajax({
			...
			success: result => {
				...
				else dashboard({ icon: 'error', text: "gagal menambahkan data" });
	...
	const act = () => {
		...
		$.ajax({
			...
			success: result => {
				...
				else dashboard({ icon: 'error', text: "gagal mengubah data" });
