...
function getDataForDisplay($collum = null, $id = null)
{
    ...
    if ($id != null && $collum != null) {
        ...
        $sql = '
            SELECT 
                data.*,  
                tingkat.title as nama_tingkat, 
                jenis.title as nama_jenis, 
                bahasa.title as nama_bahasa
            FROM 
                `data`
            INNER JOIN 
                `tingkat` 
            ON 
                `tingkat`.`id`= `data`.`id_tingkat`
            INNER JOIN 
                `jenis` 
            ON 
                `jenis`.`id`  = `data`.`id_jenis`
            INNER JOIN 
                `bahasa` 
            ON 
                `bahasa`.`id`  = `data`.`id_bahasa`
			';
    }
...
function postData()
{
    ...
    $sql = "INSERT INTO `data` 
				(`id_data`, `id_jenis`, `id_bahasa`, `id_tingkat`, `nama`, `catatan`, `total`, `selesai`) 
				VALUES 
				(NULL, '$id_jenis', '$id_bahasa', '$id_tingkat', '$nama', '$catatan', '$total', '$selesai')";
...
function postJenis()
{
    ...
    $sql = "INSERT INTO `jenis` (`id`, `title`) VALUES (NULL, '$nama_jenis')";
    if (mysqli_query($conn, $sql)) {
        responseResult([], 201, true, "data was succesfully added");
    } else {
        responseResult([], 500, false, "internal server error");
...
function postTingkat()
{
    ...
    $sql = "INSERT INTO `tingkat` (`id`, `title`) VALUES (NULL, '$nama_tingkat')";
    if (mysqli_query($conn, $sql)) {
        responseResult([], 201, true, "data was succesfully added");
    } else {
        responseResult([], 500, false, "internal server error");
...
function postBahasa()
{
    ...
    $sql = "INSERT INTO `bahasa` (`id`, `title`) VALUES (NULL, '$nama_bahasa')";
    if (mysqli_query($conn, $sql)) {
        responseResult([], 201, true, "data was succesfully added");
    } else {
        responseResult([], 500, false, "internal server error");
...
function putJenis()
{
    ...
    $id = $_POST['id'];
    $nama_jenis = $_POST['nama'];
    $sql = "UPDATE `jenis` SET `title` = '$nama_jenis' WHERE `id`='$id'";
    if (mysqli_query($conn, $sql)) {
        responseResult([], 202, true, "data has been changed");
    } else {
        responseResult([], 500, false, "internal server error");
...
function putTingkat()
{
    ...
    $id = $_POST['id'];
    $nama_tingkat = $_POST['nama'];
    $sql = "UPDATE `tingkat` SET `title` = '$nama_tingkat' WHERE `id`='$id'";
    if (mysqli_query($conn, $sql)) {
        responseResult([], 202, true, "data has been changed");
    } else {
        responseResult([], 500, false, "internal server error");
...
function putBahasa()
{
    ...
    $id = $_POST['id'];
    $nama_bahasa = $_POST['nama'];
    $sql = "UPDATE `bahasa` SET `title` = '$nama_bahasa' WHERE `id`='$id'";
    if (mysqli_query($conn, $sql)) {
        responseResult([], 202, true, "data has been changed");
    } else {
        responseResult([], 500, false, "internal server error");
